...
const CenterButton = ({ onPress }) => {
  ...
  return (
    <TouchableOpacity style={styles.centerTabButton} onPress={onPress} activeOpacity={0.7}>
      <Image source={require('../../assets/mygas_logo.png')} style={{width: '65%', height: '65%'}}/>
    </TouchableOpacity>
  );
};
  ...
  return (
    <View style={styles.tabBar}>
      {state.routes.map((route, index) => {
        const iconName =
          route.name === "Services" ? icons.services :
          route.name === "Rewards" ? icons.rewards :
          route.name === "Home" ? "home" :
          route.name === "Stations" ? icons.station :
          route.name === "Activity" ? icons.activity :
          "ellipse";
        ...
        if (route.name === "Home") {
          return (
            <View key={index} style={styles.tabButton}>
              ...
              <Text style={{marginTop: 25, color: isFocused ? "#E63946" : "#555"}}>{route.name}</Text>
            </View>
          );
        }
        ...
        return (
          ...
          >
            ...
            <Image source={iconName} style={{tintColor: isFocused ? "#E63946" : "#555", width: 25, height: 25}}/>
            <Text style={{color: isFocused ? "#E63946" : "#555"}}>{route.name}</Text>
          </TouchableOpacity>
        );
      })}
    </View>
